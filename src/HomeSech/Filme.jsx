...
const Filme = () => {
  const { films, filmsLoading, ratings, ratingsLoading } = useSelector(state => state.movies);
  ...
  useEffect(() => {
    if (films.length > 0) {
      // Only fetch ratings for the first 5 films that will be displayed
      const displayedFilms = films.slice(0, 5);
      const workIds = displayedFilms.map(film => film._id);
