...
import { fetchSeries, fetchAverageRatings } from '../redux/moviesSlice';
import 'swiper/css';
...
const Series = () => {
  const { series, seriesLoading, ratings, ratingsLoading } = useSelector(state => state.movies);
  ...
  useEffect(() => {
    if (series.length > 0) {
      // Only fetch ratings for the last 5 series that will be displayed
      const displayedSeries = series.slice(-5);
